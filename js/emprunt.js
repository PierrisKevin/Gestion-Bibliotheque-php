...
add_btn.addEventListener("click", ()=>{
    ...
    titre.innerHTML = "Nouvelle emprunt"
...
inputs[2].addEventListener("input", ()=>{
    inputs[1].value = capitalize(inputs[1].value)
})
console.log(inputs)
edits.forEach(edit => {
    edit.addEventListener("click", (e)=>{
        const maticule = e.target.id
        inputs[5].value = maticule
        modifiaction_ajout.value = "edit"
        titre.innerHTML = "Modification de l'emprunt"
        overlay.classList.remove("hidden")
        modal.classList.remove("hidden")
        const allData = e.target.parentNode.parentNode.parentNode.childNodes
        inputs[0].value = allData[0].textContent
        inputs[1].value = document.querySelector(".code_livre").value
        inputs[2].value = allData[5].textContent
        inputs[3].value = allData[6].textContent
        inputs[4].value = document.querySelector(".nombre_exemplaire").value
